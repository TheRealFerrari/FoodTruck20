...
createApp.controller('customerCreateCtrl', function($scope, $http) {
	...
	$scope.createCustomer = function() {	
		...
		} else if ($scope.newCustomerId.length != 3) {
			$scope.idMessage = "id must be  characters";
		} else {
			$scope.idMessage = "";
		}
		...
		} else if ($scope.newFoodTruckName.length < 4) {
		...
		if ($scope.idMessage == "" && $scope.nameMessage == "" && $scope.cityMessage == "")
		{
			...
			$http.post("/RestApiApp/rest/customer", newCustomer)
			.then(
					function success(response) {
